...
ZBUS_CHAN_DEFINE(pair_central_to_channel, struct nus_central_event, NULL, NULL, ZBUS_OBSERVERS_EMPTY,
		 ZBUS_MSG_INIT(0));
...
K_MSGQ_DEFINE(pair_to_queue, sizeof(struct nus_central_event), 1, 4);
...
static void pair_timeout_publish(void)
{
	...
	while (1) {
		...
		ret = zbus_chan_pub(&pair_central_to_channel, &msg, K_NO_WAIT);
		if (ret) {
...
static void pair_timer_handle(struct k_timer *timer)
{
	...
	event.event = NUS_CENTRAL_EVENT_PAIR_TIMEOUT;
