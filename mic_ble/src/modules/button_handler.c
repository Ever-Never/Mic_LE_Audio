...
#define CONFIG_BUTTON_PUBLISH_STACK_SIZE	300
#define CONFIG_BUTTON_PUBLISH_THREAD_PRIO 	5


static void button_publish_thread(void);

ZBUS_CHAN_DEFINE(button_chan, struct button_evt, NULL, NULL, ZBUS_OBSERVERS_EMPTY,
		 ZBUS_MSG_INIT(0));
...
K_MSGQ_DEFINE(button_queue, sizeof(struct button_evt), 1, 4);
...
{
//#warning "Change this control events"
	int ret = 0;
	struct button_evt event;
	LOG_INF("Button %d pressed\r\n", button_index);
	event.button_pin = btn_cfg[button_index].btn_pin;
	event.button_action = BUTTON_PRESS;

	ret = k_msgq_put(&button_queue, (void *)&event, K_NO_WAIT);
	if(ret == -EAGAIN)
	{
...
static void button_send_event_double_pressed(uint8_t button_index)
{
	...
	struct button_evt event;
	event.button_pin = btn_cfg[button_index].btn_pin;
	event.button_action = BUTTON_DOUBLE_PRESS;
	ret = k_msgq_put(&button_queue, (void *)&event, K_NO_WAIT);
	if(ret == -EAGAIN)
	{
		LOG_WRN("Btn msg queue full");
...
static void button_send_event_tripple_pressed(uint8_t button_index)
{
	...
	struct button_evt event;
	event.button_pin = btn_cfg[button_index].btn_pin;
	event.button_action = BUTTON_TRIPPLE_PRESS;
	ret = k_msgq_put(&button_queue, (void *)&event, K_NO_WAIT);
	if(ret == -EAGAIN)
	{
		LOG_WRN("Btn msg queue full");
...
static void button_scan(uint8_t button_idx)
{
	...
	if(btn_cfg[button_idx].current_status != 0)
	{
		...
		if(btn_cfg[button_idx].current_tick > BUTTON_LONGPRESS)
		{
			...
			//LOG_INF("Button %d tripple pressed\r\n", button_index);
			struct button_evt event;
			event.button_pin = btn_cfg[button_idx].btn_pin;
			event.button_action = BUTTON_LONG_PRESS;
			ret = k_msgq_put(&button_queue, (void *)&event, K_NO_WAIT);
			if(ret == -EAGAIN)
			{
				LOG_WRN("Btn msg queue full");
