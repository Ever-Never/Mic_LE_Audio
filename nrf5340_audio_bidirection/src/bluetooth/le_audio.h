...
/*USER DEFINE*/
#define BYTECH_MANUFACTURE_ID 0x1234ABCDu
#define BYTECH_DEVICE_NAME	"LavalierMicrophone"
/**/
...
typedef enum
{
	LE_AUDIO_DEVICE_GATEWAY = 0,
	LE_AUIDO_DEVICE_MIXER = 1
}le_audio_device_t;
...
typedef union 
{
	...
	struct
	{
		...
		le_audio_device_t  device_type;
		uint8_t p_manufacture_data[32];
	}refined;
	uint8_t raw[40];
}le_audio_adv_data_t;
...
/**
 * @brief Start scanning the ble audio
 *
 * @return	0 for success, error otherwise
 */
int le_audio_start_scan(void);
/**
 * @brief Start advtising audio data packet
 *
 * @return	0 for success, error otherwise
 */
int le_audio_start_adv(void);
/**
 * @brief Stop advertising audio data packet
 *
 * @return	0 for success, error otherwise
 */
