...
static void user_parser_advertising_data(struct net_buf_simple *p_ad, const bt_addr_le_t *addr)
{
	struct net_buf_simple temp_ad = *p_ad;
	uint8_t *p_data = NULL;
	uint8_t len = 0;
	uint8_t type = 0;
	...
	while(total_read <= temp_ad.len)
	{
		...
		le_audio_adv_data_t *p_data_adv = (le_audio_adv_data_t*)p_data;
		//memcpy(&p_data_adv, p_data, sizeof(p_data_adv));
		if(p_data_adv->refined.device_type == LE_AUIDO_DEVICE_MIXER &&
...
/*start scanning when press button*/
