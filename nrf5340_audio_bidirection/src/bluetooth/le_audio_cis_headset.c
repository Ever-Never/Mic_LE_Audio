...
static void advertising_process(struct k_work *work)
{
...
#endif /* CONFIG_BT_BONDABLE */
	{
		...
		/**/
		//memset(user_ad_packet, 0, sizeof(user_adpacket));
		// struct bt_data pair_data = 
		// {
		// 	.data_len = sizeof(le_audio_adv_data_t),
		// 	.type = BT_DATA_MANUFACTURER_DATA,
		// 	.data = manufacture_data
		// };
		//memcpy(pair_data.data, packet, sizeof(packet));
		le_audio_adv_data_t packet =
		{
			/* data */
			.refined.device_type = LE_AUIDO_DEVICE_MIXER,
			.refined.manufacture_id = 0x1234ABCD
		};
		uint8_t device_name[64];
		sprintf(device_name, "LavalierMicrophone %02x-%02x-%02x-%02x-%02x-%02x", addr.a.val[0],
				addr.a.val[1],
				addr.a.val[2],
				addr.a.val[3],
				addr.a.val[4],
				addr.a.val[5]);
		strcpy(packet.refined.p_manufacture_data, "LavalierMicrophone");
		LOG_INF("Device name:%s\r\n", device_name);
		struct bt_data user_peer[] = {
			BT_DATA_BYTES(BT_DATA_FLAGS, (BT_LE_AD_GENERAL | BT_LE_AD_NO_BREDR)),
			// BT_DATA_BYTES(BT_DATA_UUID16_ALL, BT_UUID_16_ENCODE(BT_UUID_ASCS_VAL)),
			// BT_DATA_BYTES(BT_DATA_UUID16_ALL, BT_UUID_16_ENCODE(BT_UUID_PACS_VAL)),
			//BT_CSIS_DATA_RSI(csis_rsi),
...
int le_audio_stop_adv()
{
	int ret = 0;
	ret = bt_le_ext_adv_stop(&adv_work);
}
