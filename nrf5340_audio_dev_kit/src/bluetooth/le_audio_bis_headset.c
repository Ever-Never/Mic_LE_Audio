...
#define LOG_DEBUG_BROADCAST	LOG_INF
...
static void get_codec_info(const struct bt_codec *codec, struct audio_codec_info *codec_info)
{
	if (codec->id == BT_CODEC_LC3_ID) {
		...
		LOG_DEBUG_BROADCAST("Retrieve the codec configuration for LC3");
			 ...
			 uint32_t broadcast_id)
{
	...
	if (strncmp(name, CONFIG_BT_DEVICE_NAME, DEVICE_NAME_PEER_LEN) == 0) {
		//LOG_INF("Broadcast source  found", name);
		return true;
	}
			 ...
			 uint32_t broadcast_id)
{
	...
	LOG_DEBUG_BROADCAST("PA synced for broadcast sink with broadcast ID 0x%06X", broadcast_id);

	broadcast_sink = sink;

	LOG_DEBUG_BROADCAST("Broadcast source PA synced, waiting for BASE");
}
...
static void pa_sync_lost_cb(struct bt_audio_broadcast_sink *sink)
{
	...
	LOG_DEBUG_BROADCAST("Sink disconnected");

	ret = bis_headset_cleanup(true);
	if (ret) {
		LOG_ERR("Error cleaning up err :%d", ret);
		return;
	}
...
static void base_recv_cb(struct bt_audio_broadcast_sink *sink, const struct bt_audio_base *base)
{
	...
	LOG_DEBUG_BROADCAST("Received BASE with %zu subgroup(s) from broadcast sink", base->subgroup_count);
	...
	for (int i = 0; i < base->subgroup_count; i++) {
		for (int j = 0; j < base->subgroups[i].bis_count; j++) {
			...
			LOG_DEBUG_BROADCAST("BIS %d   index = %hhu", j, index);
			...
			if (bitrate_check((struct bt_codec *)&base->subgroups[i].codec)) {
				...
				LOG_DEBUG_BROADCAST("Stream %d in subgroup %d from broadcast sink",
					sync_stream_cnt, i);
	...
	if (suitable_stream_found) {
		...
		LOG_DEBUG_BROADCAST("Channel %s active",
			((active_stream_index == AUDIO_CH_L) ? CH_L_TAG : CH_R_TAG));
		LOG_DEBUG_BROADCAST("Waiting for syncable");
	} else {
...
int le_audio_user_defined_button_press(void)
{
	...
	LOG_DEBUG_BROADCAST("Changed to stream %d", active_stream_index);
...
int le_audio_enable(le_audio_receive_cb recv_cb)
{
	...
	ret = bt_audio_broadcast_sink_scan_start(BT_LE_SCAN_PASSIVE);
	if (ret) {
		LOG_ERR("Broadcast sink failed err(%d)", ret);
		return ret;
	}
	LOG_DEBUG_BROADCAST("LE Audio enabled");
...
int le_audio_disable(void)
{
	...
	LOG_DEBUG_BROADCAST("LE Audio disabled");
